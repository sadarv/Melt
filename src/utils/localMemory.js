...
// ========== 核心：单次批处理调用 ==========
async function runBatch(personaId, recentMessages, userMessage, aiReply) {
  const cfg = getCfg();
  ...
  const contextLines = recentMessages
    .slice(-8)
    .map(
      (m) => `${m.role === "user" ? "用户" : "AI"}: ${m.content.slice(0, 80)}`,
    )
    .join("\n");
...
${
  enabledParts.includes("memory")
    ? `  "memory": {
    "worthy": true/false,
    "content": "关于用户的1-2条核心信息，每条10字以内，换行分隔（无则为null）"
  },`
    : '  "memory": {"worthy": false},'
}
${
  enabledParts.includes("fragments")
    ...
    {"content": "碎片内容（15字内）", "confidence": 0.6}
  ],`
    : '  "fragments": [],'
}
...
- memory：只提取值得长期记住的事——重要约定、情感转折、关系里程碑、用户说过的触动心弦的话。不记录日常闲聊。每条不超过15字。
- fragments：提取1-3条独立事实，每条带置信度（0.6-1.0，确定的事实给高分，推测或模糊的给低分）。已有碎片：${recentFragments || "无"}，不重复
- graph_node：判断这段对话是否有值得作为记忆节点的内容，AI自由决定主题，已有主题：${existingNodeThemes || "无"}
...
async function dispatchBatchResult(personaId, result) {
  ...
  if (result.timeline?.worthy && result.timeline.content) {
    ...
    const recentTimeline = (await storage.getTimeline(personaId))
      .slice(0, 3)
      .map((e) => e.content);
    if (!recentTimeline.some((c) => c.includes(content.slice(0, 10)))) {
  ...
  if (result.memory?.worthy && result.memory.content) {
    await storage.saveMemory(personaId, { content: result.memory.content });
    console.log(`[LocalMemory] 记忆: ${result.memory.content.slice(0, 30)}`);
  }
  ...
  if (result.graph_node?.worthy && result.graph_node.title) {
    ...
    const existing = nodes.find(
      (n) => n.theme === node.theme || n.title === node.title,
    );
    ...
    for (const n of nodes) {
      const currentNode = existing || nodes[nodes.length - 1];
      if (n.id === currentNode.id) continue;
      const overlap = (node.keywords || []).filter((k) =>
        (n.keywords || []).includes(k),
      ).length;
      if (overlap >= 1) {
        if (!currentNode.linked_themes.includes(n.theme))
          currentNode.linked_themes.push(n.theme);
        if (!n.linked_themes.includes(node.theme))
          n.linked_themes.push(node.theme);
      }
    }
  ...
  if (result.corpus?.worthy && Array.isArray(result.corpus.items)) {
    ...
    if (result.corpus.items.length > 0) {
      console.log(`[LocalMemory] 语料: ${result.corpus.items.length} 条`);
    }
  }
}
...
export async function manualSediment(personaId) {
  ...
  const result = await runBatch(personaId, recent, userMsg, aiMsg);
